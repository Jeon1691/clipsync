...
echo "  CLIPSYNC_HOME=$A cargo run -p clipsync-cli -- init"
echo "  CLIPSYNC_HOME=$B cargo run -p clipsync-cli -- init"
echo "  CLIPSYNC_HOME=$A cargo run -p clipsync-cli -- room create --no-auto-sync   # prints code, waits in background"
